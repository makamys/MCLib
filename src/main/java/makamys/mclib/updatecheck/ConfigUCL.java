...
public class ConfigUCL {
    ...
    public static void reload() {
        Configuration config = new Configuration(
            new File(Launch.minecraftHome, "config/" + UpdateCheckLib.MODID + ".cfg"));

        config.load();

        enabled = config.getBoolean("enable", "_general", true, "Set this to false to disable this mod");
        hideErrored = config
            .getBoolean("hideErrored", "_general", false, "If true, results that errored won't be shown");
        promoChannel = config.getString(
            "promoChannel",
            "_general",
            "latest",
            "The update channel which should be checked. Forge defines 'latest' for the bleeding-edge versions and 'recommended' for the stable versions of mods.");

        showUpdatesButton = config.getBoolean(
            "showUpdatesButton",
            "interface",
            true,
            "Show updates button in main menu when there are updates available");
        ...
        updatesButtonAbsolutePos = config.getBoolean(
            "updatesButtonAbsolutePos",
            "interface",
            false,
