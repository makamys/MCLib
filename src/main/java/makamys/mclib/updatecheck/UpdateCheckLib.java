...
public class UpdateCheckLib {
    ...
    static UpdateCategory RESOURCE_PACKS = new UpdateCategory(
        UpdateCheckAPI.RESOURCE_PACKS_CATEGORY_ID,
        Loader.MC_VERSION,
        "Resource pack",
        false);
    ...
    public void onGui(InitGuiEvent.Post event) {
        if (event.gui instanceof GuiMainMenu) {
            ...
            if (ConfigUCL.showUpdatesButton) {
                ...
                int buttonY = ConfigUCL.updatesButtonY
                    + (ConfigUCL.updatesButtonAbsolutePos ? 0 : event.gui.height / 4);
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        CompletableFuture.allOf(futures.toArray(new CompletableFuture[] {}))
            .thenRun(new Runnable() {
                ...
                @Override
                public void run() {
                    List<UpdateCheckTask.Result> results = futures.stream()
                        .map(f -> {
                            try {
                                return f.get();
                            } catch (Exception e) {
