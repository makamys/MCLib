...
public class ResultHTMLRenderer {
    ...
    private static final String FIELD_NAME = "Name", FIELD_CURRENT_VERSION = "Installed version",
        FIELD_NEW_VERSION = "Latest version", FIELD_URL = "Update link", TABLE_TITLE_TEMPLATE = "%s updates";
    ...
    private boolean hasAnythingToDisplay() {
        return UpdateCheckLib.categories.values()
            .stream()
            .anyMatch(
                cat -> cat.results.stream()
                    .anyMatch(r -> r.isInteresting()));
    }
        ...
        if (!hasAnythingToDisplay()) {
            ...
            try (FileOutputStream out = new FileOutputStream(outFile)) {
                String template = IOUtils.toString(
                    ResultHTMLRenderer.class.getClassLoader()
                        .getResourceAsStream(
                            "resources/mclib/" + MCLib.RESOURCES_VERSION + "/updatecheck/updates.template.html"));
    ...
    private String generateTables() {
        ...
        Stream.concat(
            first.stream(),
            UpdateCheckLib.categories.values()
                .stream()
                .sorted()
                .filter(c -> !first.contains(c)))
            .forEach(cat -> {
                List<UpdateCheckTask.Result> interestingResults = cat.results.stream()
                    .filter(r -> r.isInteresting())
                ...
                if (!interestingResults.isEmpty()) {
                    ...
                    for (UpdateCheckTask.Result result : interestingResults) {
                        String newVersionStr = result.newVersion != null ? htmlEscape(result.newVersion.toString())
                            : "<b>ERROR</b>";
                        String homepagesStr = String.join(
                            TABLE_HOMEPAGE_SEPARATOR,
                            result.task.homepages.stream()
                                .map(
                                    hp -> String
                                        .format(TABLE_HOMEPAGE_TEMPLATE, urlEscape(hp.url), htmlEscape(hp.display)))
                                .toArray(String[]::new));
                        rows += String.format(
                            TABLE_ROW_TEMPLATE,
                            htmlEscape(result.task.name),
                            htmlEscape(result.task.currentVersion.toString()),
                            newVersionStr,
                            homepagesStr);
                    }
                    ...
                    tables.append(
                        String.format(
                            TABLE_TEMPLATE,
                            String.format(TABLE_TITLE_TEMPLATE, htmlEscape(tableTitle)),
                            FIELD_NAME,
