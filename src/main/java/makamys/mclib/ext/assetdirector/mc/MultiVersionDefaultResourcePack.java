...
public class MultiVersionDefaultResourcePack implements IResourcePack {
    ...
    private static final boolean DUMP_SOUNDS_JSON = Boolean
        .parseBoolean(System.getProperty("assetDirector.dumpSoundsJson", "false"));
    ...
    public Set getResourceDomains() {
        return fetcher.versionIndexes.keySet()
            .stream()
            .map(v -> "minecraft_" + v)
    ...
    public boolean resourceExists(ResourceLocation resLoc) {
        ...
        return scratch.isValid
            && (scratch.isInJar ? true : fetcher.hashExists(scratch.hash) || scratch.mcResPack != null);
    }

    private void parseName(ResourceLocation resLoc) {
        if (resLoc.equals(scratch.resLoc)) return;

        scratch.isValid = resLoc.getResourceDomain()
            .startsWith("minecraft_");
        if (!scratch.isValid) {
            return;
        }
        ...
        if (!resLoc.getResourcePath()
            .equals("sounds.json")) {
            ResourceLocation mcResLocOld = new ResourceLocation(
                "minecraft",
                convertPath(resLoc.getResourcePath(), v1_7_10));
            ResourceLocation mcResLocNew = new ResourceLocation(
                "minecraft",
                convertPath(resLoc.getResourcePath(), v1_13));
    ...
    public static void inject(AssetDirector assetDirector) {
        ...
        List defaultResourcePacks = ReflectionHelper
    ...
    private void stripUnusedSounds(JsonObject soundsJSON, AssetIndex index) {
        if (soundsJSON.has(SOUNDS_JSON_REQUESTED)) {
            Set<String> requested = StreamSupport.stream(
                soundsJSON.getAsJsonArray(SOUNDS_JSON_REQUESTED)
                    .spliterator(),
                false)
                .map(e -> e.getAsString())
                .collect(Collectors.toSet());
            ...
            soundsJSON.entrySet()
                .removeIf(sound -> !requested.contains(sound.getKey()));
        }
    }
