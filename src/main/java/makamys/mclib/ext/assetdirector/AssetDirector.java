...
public class AssetDirector {
    ...
    private void parseJson(String json, String modid) throws Exception {
        ...
        objectFetchQueue = objectFetchQueue.stream()
            .filter(fetcher::needsFetchAssetByHash)
            .collect(Collectors.toSet());
        jarFetchQueue = jarLoadQueue.stream()
    ...
    private List<String> getObjectsAndSetCategories(Collection<SoundEvent> soundEvents, JsonObject soundJson,
        String modid) {
        ...
        requested = soundJson.get(SOUNDS_JSON_REQUESTED)
            .getAsJsonArray();
    ...
    private JsonObject getOrFetchSoundJson(String version) throws Exception {
        ...
        if (soundJson == null) {
            ...
            soundJson = new Gson().fromJson(
                new InputStreamReader(
                    fetcher.getAssetInputStream(version, "minecraft/sounds.json"),
    ...
    private static File getAssetDirectorDir() {
        ...
        // The old launcher deletes extra files from the assets directory, so we can't live there. Use `<launcher work
        // dir>/asset_director` instead.
        return !isOldLauncher(assetsDir) ? new File(assetsDir, "asset_director")
            : new File(assetsDir, "../asset_director");
    }
            ...
            try (FileReader fr = new FileReader(launcherJson)) {
                ...
                if (launcherVersionObj != null) {
                    ...
                    if (name.isString()) {
                        ...
                        LOGGER.debug(
                            "Detected official launcher (version " + launcherVersion
                                + "). Will use alternative directory location.");
