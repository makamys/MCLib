 ...
 * Needed because java 1.8.0_51 doesn't support Mojang's new certificates, and many launchers bundle this version of
 * Java.
 */
...
public class SSLHacker {
            ...
            if (Integer.parseInt(patchVersion) < 75) {
                LOGGER.warn(
                    "Your Java version (" + javaVersion
                        + ") is out of date! Please consider updating to a newer version for improved stability and security.");
    ...
    private static void replaceSSLContext() throws Exception {
        ...
        try (InputStream is = SSLHacker.class.getClassLoader()
            .getResourceAsStream(
                "resources/mclib/" + MCLib.RESOURCES_VERSION + "/certs/DigiCertGlobalRootG3.crt.pem")) {
            final Certificate cert = cf.generateCertificate(is);
            keyStore.setCertificateEntry("digicertglobalrootg3", cert);
        }
        // TODO: consider adding other root certs added in
        // https://www.oracle.com/java/technologies/javase/8u75-relnotes.html
