...
public class ResourcePackUtil {
    ...
    public static List<IResourcePack> getMinecraftResourcePackList() {
        if (domainResourceManagers == null) {
            ...
            if (resMan instanceof SimpleReloadableResourceManager) {
                domainResourceManagers = (Map<String, IResourceManager>) ReflectionHelper.getPrivateValue(
                    SimpleReloadableResourceManager.class,
                    (SimpleReloadableResourceManager) resMan,
        ...
        if (newMcResMan == mcResMan) {
            ...
            if (newMcResMan instanceof FallbackResourceManager) {
                newMcResPacks = ReflectionHelper.getPrivateValue(
                    FallbackResourceManager.class,
                    (FallbackResourceManager) newMcResMan,
                    "resourcePacks",
                    "field_110540_a");
            } else {
    ...
    /**
     * Returns true if the resource pack is one that should be ignored by AssetDirector when searching for overrides of
     * modern assets.
     */
    public static boolean isBuiltIn(IResourcePack resPack) {
        return resPack instanceof DefaultResourcePack || resPack.getPackName()
            .equals("FMLFileResourcePack:Forge Mod Loader")
            || resPack.getPackName()
                .equals("FMLFileResourcePack:Minecraft Forge");
    }
