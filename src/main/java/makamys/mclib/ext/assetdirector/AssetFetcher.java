...
public class AssetFetcher {
    ...
    final static String MANIFEST_ENDPOINT = System
        .getProperty("assetDirector.manifestEndpoint", "https://launchermeta.mojang.com/mc/game/version_manifest.json");
    final static String RESOURCES_ENDPOINT = System
        .getProperty("assetDirector.resourcesEndpoint", "https://resources.download.minecraft.net");
    ...
    private static final int DOWNLOAD_TIMEOUT = Integer
        .parseInt(System.getProperty("assetDirector.downloadTimeout", "10000")), // ms
    ...
    public void init() {
        ...
        Arrays.stream(adDir.listFiles((dir, name) -> name.endsWith(".part")))
            .forEach(f -> f.delete());
    ...
    private void downloadAssetByHash(String hash) throws IOException {
        ...
        for (int i = 0; i < DOWNLOAD_ATTEMPTS; i++) {
            ...
            if (hash.equals(getSha1(outFileTmp))) {
                ...
                LOGGER.warn(
                    "Got invalid hash when downloading " + hash + ". Attempt " + (i + 1) + "/" + DOWNLOAD_ATTEMPTS);
            }
        }
    }
    ...
    public void loadVersionDeps(String version) throws Exception {
        ...
        if (!assetIndexes.containsKey(vi.assetsId)) {
            ...
            if (!assetIndex.exists()) {
                String url = vi.json.get("assetIndex")
                    .getAsJsonObject()
                    .get("url")
                    .getAsString();
    ...
    private void downloadVersionIndex(String version, File dest) throws Exception {
        ...
        for (JsonElement verElem : manifest.get("versions")
            .getAsJsonArray()) {
    ...
    private <T> T loadJson(InputStream stream, Class<T> classOfT) throws Exception {
        return new Gson()
            .fromJson(new InputStreamReader(new BufferedInputStream(stream), StandardCharsets.UTF_8), classOfT);
    }
