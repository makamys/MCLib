...
public class ADTest {
    ...
    public void onConstruction(FMLConstructionEvent event) {
        ...
        config.addObject("1.17", "minecraft/sounds/item/bucket/fill_axolotl1.ogg"); // Test downloading single sound
                                                                                    // file (check the example
                                                                                    // sounds.json for how to use)
        config.addJar("1.17");
    ...
    public void onPreInit(FMLPreInitializationEvent event) {
        if (event.getSide() == Side.CLIENT) {
            ...
            try {
                JsonObject lang_1_17 = new Gson().fromJson(
                    new BufferedReader(
                        new InputStreamReader(
                            Minecraft.getMinecraft()
                                .getResourceManager()
                                .getResource(new ResourceLocation("minecraft_1.17:lang/ms_my.json"))
                                .getInputStream(),
                            Charsets.UTF_8)),
                    JsonObject.class);
                // Item added in 1.13
                ...
                JsonObject lang_1_19_3 = new Gson().fromJson(
                    new BufferedReader(
                        new InputStreamReader(
                            Minecraft.getMinecraft()
                                .getResourceManager()
                                .getResource(new ResourceLocation("minecraft_1.19.3:lang/nah.json"))
                                .getInputStream(),
                            Charsets.UTF_8)),
                    JsonObject.class);
