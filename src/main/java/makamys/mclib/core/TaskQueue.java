...
public class TaskQueue {
    ...
    static Map<LoaderState, Map<String, Triple<ComparableVersion, Object, Runnable>>> queuedTasks = SharedReference
        .get("TaskQueue", "queuedTasks", HashMap.class);

    /**
     * Enqueues a task to get executed the next time {@link state} is reached. If a task with the same name is already
     * registered, we will only overwrite it if our version is greater.
     */
    public static void enqueueTask(LoaderState state, String taskName, Runnable task, ComparableVersion version) {
        ...
        if (versionAndTask == null || versionAndTask.getLeft()
            .compareTo(version) < 0) {
