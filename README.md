...
* [`AssetDirector`](https://github.com/makamys/MCLib/wiki/AssetDirector): downloader for Mojang's assets and Minecraft jars directly off their servers, allowing use and redistribution of things like sounds without EULA worries.
	...
	* Supported Minecraft download URLs can be redirected to a mirror by setting `-DassetDirector.downloadRedirectBaseUrl=<base url>` or by calling `AssetDirectorAPI.setDownloadRedirectBaseUrl(String)` before pre-init. For example, `-DassetDirector.downloadRedirectBaseUrl=https://bmclapi2.bangbang93.com` redirects network downloads while keeping the local Mojang-compatible cache layout unchanged.
